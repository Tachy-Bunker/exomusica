import { useEffect, useRef } from "react";
import { useSiteEffectsStore } from "../lib/siteEffectsStore";

const BASE_TILE_PX = 200;

function triangleWave(phase01: number): number {
  // 0..1 input, 0..1 output, linear ramp up then down — phase01 is the
  // fraction of one full cycle completed.
  const p = phase01 % 1;
  return p < 0.5 ? p * 2 : 2 - p * 2;
}

export function MoireLayer() {
  const ref = useRef<HTMLDivElement>(null);

  useEffect(() => {
    let rafId: number;
    let paused = document.hidden;

    function handleVisibility() {
      paused = document.hidden;
      if (!paused) rafId = requestAnimationFrame(tick);
    }
    document.addEventListener("visibilitychange", handleVisibility);

    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    function tick(now: number) {
      if (paused) return;
      const el = ref.current;
      if (el) {
        const s = useSiteEffectsStore.getState();
        const t = now / 1000;

        if (!reduceMotion) {
          const cyclePhase = t * s.moireOffsetSpeed;
          const wave = s.moireWaveform === "triangle" ? triangleWave(cyclePhase) : (Math.sin(cyclePhase * Math.PI * 2) + 1) / 2;
          const offset = s.moireOffsetMin + wave * (s.moireOffsetMax - s.moireOffsetMin);
          const rotation = (t * s.moireRotationSpeed * 360) % 360; // degrees per second * t, wrapped

          el.style.backgroundPosition = `${offset}px ${offset}px`;
          el.style.transform = `rotate(${rotation}deg)`;
        }

        el.style.opacity = String(s.moireOpacity);
        el.style.backgroundImage = s.moireImageUrl ? `url(${s.moireImageUrl})` : "none";
        el.style.backgroundSize = `${BASE_TILE_PX * s.moireSize}px`;
      }
      rafId = requestAnimationFrame(tick);
    }
    rafId = requestAnimationFrame(tick);

    return () => {
      cancelAnimationFrame(rafId);
      document.removeEventListener("visibilitychange", handleVisibility);
    };
  }, []);

  return (
    <div
      ref={ref}
      style={{
        position: "fixed",
        inset: "-50%",
        width: "200%",
        height: "200%",
        zIndex: 9998,
        pointerEvents: "none",
        backgroundRepeat: "repeat",
        mixBlendMode: "overlay",
      }}
    />
  );
}
