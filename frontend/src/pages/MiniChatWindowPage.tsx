...
export function MiniChatWindowPage() {
  ...
  const [followDock, setFollowDock] = useState(true);
  const [manualSlug, setManualSlug] = useState<string | null>(initialSlug);
  const [manualName, setManualName] = useState<string | null>(initialName);
  ...
  useEffect(() => {
    ...
    setResults(allChannels.filter((c) => c.name.toLowerCase().includes(q)).slice(0, 15));
  }, [query, allChannels]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", fontFamily: "var(--font-body)", color: "var(--text)", background: "var(--bg)" }}>
      <div style={{ padding: "0.4rem 0.6rem", borderBottom: "1px solid var(--border)", display: "flex", flexDirection: "column", gap: "0.3rem" }}>
        ...
        {!followDock && (
          <div style={{ position: "relative" }}>
            ...
            />
            {results.length > 0 && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 5, background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "var(--radius)", maxHeight: 160, overflowY: "auto" }}>
                {results.map((c) => (
                  <div
                    key={c.slug}
                    onClick={() => {
